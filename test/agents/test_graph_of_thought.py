...
from assistant.agents.graph_of_thoughts import controller, operations, parser
from assistant.agents.prompts import prompter
from assistant.llm import language_models


class DocMergePrompter(prompter.Prompter):
    merge_doc_prompt_start = '''Merge the following {num} NDA documents <Doc1>
     - <Doc{num}> into a single NDA, maximizing retained information and minimizing
     redundancy. Output only the created NDA between the tags <Merged> and </Merged>,
    ...
    '''
    merge_doc_prompt_cot_start = '''Merge the following {num} NDA documents <Doc1>
     - <Doc{num}> into a single NDA, maximizing retained information and minimizing
     redundancy.You can generate any intermediate thoughts and documents you want,
     but the final output should be the merged NDA, placed between the two tags <Merged>
     and </Merged>.
     For instance you might want to follow this approach:
     1. Split each NDA into their logical sub-parts.
     2. Merge the sub-parts of the {num} NDAs.
     3. Combine the merged sub-parts into a single NDA.
     4. Place the merged NDA between the tags <Merged> and </Merged>.
     
     Here are NDAs <Doc1> - <Doc{num}>:
    '''
    improve_summary_prompt_start = '''The following NDA <S> merges initial NDAs <Doc1> - 
    <Doc{num}>.Please improve the summary NDA <S> by adding more information and removing
    redundancy.Output only the improved NDA, placed between the tags <Merged> and </Merged>,
    without any additional text.
    ...
    Please score the merged NDA <S> in terms of how much redundant information is contained, 
    independent of the original NDAs, as well as how much information is retained from 
    the original NDAs.
    A score of 10 for redundancy implies that absolutely no information is redundant, 
    while a score of 0 implies that at least half of the information is redundant 
    (so everything is at least mentioned twice).
    A score of 10 for retained information implies that all information from the original 
    NDAs is retained, while a score of 0 implies that no information is retained.
    You may provide reasoning for your scoring, but the final score for redundancy should 
    be between the tags <Redundancy> and </Redundancy>, and the final score for retained 
    information should be between the tags <Retained> and </Retained>, without any additional 
    text within any of those tags.
    ...
    '''
    aggregate_full_prompt_base = '''The following NDAs <S1> - <S{num_ndas_summary}> each 
    merge the initial NDAs <Doc1> - <Doc{num_ndas}>.
    Combine the merged NDAs <S1> - <S{num_ndas_summary}> into a new one, maximizing their 
    advantages and overall retention, while minimizing redundancy.
    Output only the the new NDA between the tags <Merged> and </Merged>, without any additional
    text.
    ...
    '''
    aggregate_sub_prompt_base = '''The following NDAs <S1> - <S{num_ndas}> are summaries of 
    some other NDAs.
    Combine them into a new one, make sure to maximize their advantages and overall information 
    retention, while minimizing redundancy. 
    ...
    def aggregation_prompt(
        ...
        if len(state_dicts[0]["parts"]) > 0 and len(state_dicts[0]['parts']) < len(
            ...
            prompt = self.aggregate_sub_prompt_base.format(num_ndas=len(state_dicts),)
    ...
    def strip_answer_helper(self, text: str, tag: str = '') -> str:
        ...
        if tag != -1:
            ...
            if start != -1 and end != -1:
                ...
                logging.warning(
                    f"Only found the start tag <{tag}> in answer: {text}. Returning everything after the atg."
                )
    ...
    def parse_score_answer(
        ...
        if len(states) == 1:
            ...
            if len(redundancy_scores) == 0 or len(retain_scores) == 0:
                logging.warning(f"Could not find any valid score in any answer. Returning 0.0.")
                return [0.0]
...
def io() -> operations.GraphOfOperations:
    ...
    operations_graph.append_operation(operations.Generate(1,
                                                          1))
    operations_graph.append_operation(operations.Score(3,
                                                       False))
...
def run(
    ...
    extra_info = f"{lm_name}_{'_'.join([method.__name__ for method in methods])}"
        ...
        for method in methods:
            ...
            lm = language_models.ChatGPT(
                os.path.join(
                    ...
                    "../../../assistant/agents/graph_of_thoughts/language_models/config.json",
                ),
                model_name=lm_name,
                cache=True,
            )
