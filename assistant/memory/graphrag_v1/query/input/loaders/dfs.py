...
from assistant.memory.graphrag_v1.query.input.loaders.utils import (
    to_str,
    to_list,
    to_optional_int,
    to_optional_str,
...
def read_entities(
    ...
    for idx, row in df.iterrows():
        entity = Entity(
            ...
            name_embedding=to_optional_list(
                row, name_embedding_col, item_type=float
            ),
            description_embedding=to_optional_list(
                row, description_embedding_col, item_type=float
            ),
            graph_embedding=to_optional_list(
                row, graph_embedding_col, item_type=float
            ),
...
def read_text_units(
    ...
    for idx, row in df.iterrows():
        chunk = TextUnit(
            ...
            text_embedding=to_optional_list(row, embedding_col, item_type=float),
...
def read_documents(
    ...
    for idx, row in df.iterrows():
        doc = Document(
            ...
            raw_content=to_str(row, summary_col),
            summary_embedding=to_optional_list(
                row, summary_embedding_col, item_type=float
            ),
            ...
            text_unit_ids=to_list(row, text_units_col, item_type=str),
            attributes=(
                {col: row.get(col) for col in attributes_cols}
                if attributes_cols
                else None
